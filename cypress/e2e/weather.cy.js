...
cy.contains("City:").find("b").should("have.text", "London");
  ...
  // it("can search via button click", () => {
  //   cy.get("[data-cy=city-input]").should("be.enabled").type("Rome");
  //   cy.get("[data-cy=search-button]").click();
  //   cy.contains("City:").find("b").should("have.text", "Rome");
  // });
  ...
  it("toggles weather and pomodoro panels", () => {
    ...
    cy.get(".pomodoro").should("not.exist"); 
  });
...
it("toggles music button text", () => {
  cy.get("[data-cy=toggle-music]").click();
  cy.contains("Music On").should("exist");

  cy.get("[data-cy=toggle-music]").click();
  cy.contains("Music Off").should("exist");
});
...
it("switches app language to Italian", () => {
  cy.get("select.language-dropdown").select("it");
  cy.contains("Meteo").should("exist"); 
});

it("switches app language to Spanish", () => {
  cy.get("select.language-dropdown").select("es");
...
// it("starts and resets the pomodoro timer", () => {
//   cy.get("[data-cy=toggle-pomodoro]").click();
//   cy.get("[data-cy=pomodoro]").should("exist");

//   cy.get("[data-cy=pomodoro-time]")
//     .invoke("text")
//     .then((initial) => {
//       cy.get("[data-cy=pomodoro-startstop]").click(); // start
//       cy.wait(2500); // wait > 2s to guarantee at least 1 tick
//       cy.get("[data-cy=pomodoro-time]").should("not.have.text", initial);
//     });

//   cy.get("[data-cy=pomodoro-reset]").click();
//   cy.get("[data-cy=pomodoro-time]").should("contain.text", "25");
// });
